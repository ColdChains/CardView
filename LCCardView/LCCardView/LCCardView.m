...
@interface LCCardView () <UIGestureRecognizerDelegate>

// 滚动视图
@property (nonatomic, strong) LCCardScrollView *scrollView;
...
- (LCCardScrollView *)scrollView {
    if (!_scrollView) {
        _scrollView = [[LCCardScrollView alloc] init];
...
- (void)layoutPageControl {
    ...
    switch (self.orientation) {
        case LCCardViewOrientationHorizontal:
        {
        ...
        case LCCardViewOrientationVertical:
        {
            [self.pageControl mas_remakeConstraints:^(MASConstraintMaker *make) {
                make.left.mas_equalTo(self.pageControlEdgeInsets.left);
...
- (void)initView {
    ...
    self.orientation = LCCardViewOrientationHorizontal;
    self.alignment = LCCardViewAlignmentLeft;
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    do {
        if ([self.cells containsObject:view]) {
            ...
            if (self.delegate && [self.delegate respondsToSelector:@selector(cardView:didSelectItemAtIndex:)]) {
                [self.delegate cardView:self didSelectItemAtIndex:index % self.numberOfItems];
            }
        }
        view = view.superview;
...
- (void)reloadData {
    ...
    // 设置总页数
    if (self.dataSource && [self.dataSource respondsToSelector:@selector(numberOfItemsInCardView:)]) {
        
        self.numberOfItems = [self.dataSource numberOfItemsInCardView:self];
        
        if (self.canLoop) {
            self.showNumberOfItems = self.numberOfItems == 1 ? 1: [self.dataSource numberOfItemsInCardView:self] * 3;
        } else {
            self.showNumberOfItems = self.numberOfItems == 1 ? 1: [self.dataSource numberOfItemsInCardView:self];
        }
    ...
    if (self.delegate && self.delegate && [self.delegate respondsToSelector:@selector(sizeForItemInCardView:)]) {
        self.itemSize = [self.delegate sizeForItemInCardView:self];
    }
    ...
    switch (self.orientation) {
        case LCCardViewOrientationHorizontal:
        {
            ...
            if (self.canLoop || self.alignment != LCCardViewAlignmentLeft) {
...
- (void)layoutItemAtIndex:(NSInteger)index {
    ...
    cell = [self.dataSource cardView:self cellForItemAtIndex:index % self.numberOfItems];
